...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/sap/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/sap/*")
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap/config/schemas",
    )
